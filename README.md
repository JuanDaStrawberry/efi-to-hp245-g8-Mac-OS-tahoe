...
Once macOS is installed, place the EFI partition in its designated location. Now you can connect to the internet and sign in with your iCloud account. Sign in, and you'll be prompted for the password from your other Apple device to decrypt iCloud. Enter it, let the process fail, and it will tell you that you forgot the password and to add it later. Now mount your EFI partition and edit its config.plist file. Enable Secure Boot (in this case, the value is j214k). (MacbookPro16,2) Restart your system and then return to the Apple account settings. Try syncing iCloud again, this time entering the decryption password. It should succeed. This process will enable iServices.
...
- amdpowermanagemetkext: Many EFIS include it, but in this case, it's not included because the system works quite well. Although there's a slight improvement in battery life (very minimal) with the implementation of this kext, I see a negative significant impact on Wi-Fi performance. You can try it, and if you don't detect any instability, use it.
